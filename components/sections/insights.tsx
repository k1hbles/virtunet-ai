    ...
    <section className="border-t border-line bg-canvas py-24 md:py-32">
      <div className="wrap">
        <div className="flex items-end justify-between gap-6">
          <h2 className="text-balance text-4xl font-medium tracking-[-0.04em] text-ink md:text-6xl">
            {insights.title}
          </h2>
          <a
            href={insights.cta.href}
            className="hidden items-center gap-2 text-sm text-ink transition-colors hover:text-accent md:inline-flex"
          >
            {insights.cta.label}
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-3">
          {insights.items.map((post) => (
            ...
            >
              <div className={`h-1 w-14 rounded-full ${RULE[post.rule]}`} />
              <div>
